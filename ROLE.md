...
- **Identity and authentication for suite users** — owned by `sparxstar-3iatlas-identity-node` (`sparxstar-identity`; RS256 JWT, Node + PostgreSQL). It is the platform authority root, succeeding Sirus. New user-facing endpoints must not use `wp_nonce` or `is_user_logged_in()` (the legacy frontend form predates this rule — see Open items).
...
- **DVE runtime dependencies** (Helios, Mḗh₁n̥s, Dheghom) — this repo operates in standalone mode and does not connect to any of these at runtime. Sirus is retired platform-wide: the authority root is `sparxstar-3iatlas-identity-node` (see `.claude/ref-07-authority-root.md`), and this repo holds no runtime dependency on it either.
