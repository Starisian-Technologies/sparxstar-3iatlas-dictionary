...
- **Identity and authentication for suite users** — owned by `sparxstar-3iatlas-identity-node` (`sparxstar-identity`; RS256 JWT, Node + Express + PostgreSQL — earlier drafts said Cloudflare Workers; the shipped service is not). New user-facing endpoints must not use `wp_nonce` or `is_user_logged_in()` (the legacy frontend form predates this rule — see Open items).
