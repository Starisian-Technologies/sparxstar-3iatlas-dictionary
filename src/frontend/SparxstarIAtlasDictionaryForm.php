...
final class SparxstarIAtlasDictionaryForm 
{
    ...
    public function sparxIAtlas_register_hooks() {
        add_action('wp_enqueue_scripts', [$this, 'aiwa_dict_form_enqueue_scripts']);
        add_action('wp_ajax_aiwa_dict_form_submit', [$this, 'aiwa_dict_submit_form']);
        add_action('wp_ajax_aiwa_dict_search_synonyms', [$this, 'aiwa_dict_search_synonyms']);
    }

    public function sparxIAtlas_register_shortcodes() {
        add_shortcode('aiwa_dictionary_form', [$this, 'aiwa_dictionary_render_form']);
    }

/**
 * Enqueue scripts and styles
 */
public function aiwa_dict_form_enqueue_scripts() {
    if (is_page() && has_shortcode(get_post()->post_content, 'aiwa_dictionary_form')) {
        wp_enqueue_media();
        if (defined('SPARX_IATLAS_URL')) {
            wp_enqueue_style('aiwa-dict-form-style', SPARX_IATLAS_URL . 'src/css/sparxstar-iatlas-dictionary-form.css', array(), '1.0');
            wp_enqueue_script('aiwa-dict-form-script', SPARX_IATLAS_URL . 'src/js/sparxstar-iatlas-dictionary-form.js', array('jquery'), '1.0', true);
        } else {
            // Fallback if constant not defined (e.g. during standalone testing)
            wp_enqueue_style('aiwa-dict-form-style', plugin_dir_url(dirname(__DIR__)) . 'css/sparxstar-iatlas-dictionary-form.css', array(), '1.0');
            wp_enqueue_script('aiwa-dict-form-script', plugin_dir_url(dirname(__DIR__)) . 'js/sparxstar-iatlas-dictionary-form.js', array('jquery'), '1.0', true);
        }
        
        wp_localize_script('aiwa-dict-form-script', 'aiwaDict', array(
...
public function aiwa_dictionary_render_form(array $atts): string {
    ...
    <div class="aiwa-dict-form-container">
        ...
        <form id="aiwa-dict-form" class="aiwa-dict-form" data-entry-id="<?php echo $entry_id; ?>">
                ...
                <div class="form-group">
                    ...
                    <div class="media-upload-container">
                        ...
                        <div class="image-preview-container">
                            <?php if (!empty($entry_data['word_photo'])): 
                ...
                <div id="example-sentences-container">
                    ...
                    if (!empty($sentences) && is_array($sentences)) {
                        foreach ($sentences as $index => $sentence) {
                            ?>
                            <div class="sentence-row" data-index="<?php echo $index; ?>">
                                <div class="sentence-fields">
                                    <div class="form-group">
                                        <label>Sentence</label>
                                        <input type="text" name="sentences[<?php echo $index; ?>][aiwa_sentence]" value="<?php echo esc_attr($sentence['aiwa_sentence'] ?? ''); ?>">
                                    </div>
                                    <div class="form-group">
                                        <label>Translation</label>
                                        <input type="text" name="sentences[<?php echo $index; ?>][aiwa_s_translation]" value="<?php echo esc_attr($sentence['aiwa_s_translation'] ?? ''); ?>">
                                    </div>
                                    <div class="form-group">
                                        <label>Translation (English)</label>
                                        <input type="text" name="sentences[<?php echo $index; ?>][aiwa_s_translation_english]" value="<?php echo esc_attr($sentence['aiwa_s_translation_english'] ?? ''); ?>">
                                    </div>
                                    <div class="form-group">
                                        <label>Translation (French)</label>
                                        <input type="text" name="sentences[<?php echo $index; ?>][aiwa_s_translation_french]" value="<?php echo esc_attr($sentence['aiwa_s_translation_french'] ?? ''); ?>">
                                    </div>
                                </div>
                    ...
                    <div id="selected-synonyms" class="selected-synonyms">
                        ...
                        if (!empty($entry_data['synonyms']) && is_array($entry_data['synonyms'])) {
                            foreach ($entry_data['synonyms'] as $syn_id) {
                                ...
                                if ($syn_post) {
                                    echo '<span class="synonym-tag" data-id="' . $syn_id . '">' . esc_html($syn_post->post_title) . ' <button type="button" class="remove-synonym">×</button></span>';
                                }
                            }
                        }
                        ?>
 ...
 */
public function aiwa_dict_submit_form() {
    // Verify nonce
    if (!isset($_POST['nonce']) || !wp_verify_nonce($_POST['nonce'], 'aiwa_dict_form_nonce')) {
 ...
 */
public function aiwa_dict_search_synonyms() {
    if (!isset($_POST['nonce']) || !wp_verify_nonce($_POST['nonce'], 'aiwa_dict_form_nonce')) {
        wp_send_json_error(array('message' => 'Security check failed.'));
    }
