...
class Sparxstar3IAtlasAutoLinker {
    ...
    private function get_dictionary_terms(): array {
        ...
        // Cache the full list for 90 days
    ...
    private function process_replacements( string $content, array $terms ): string {
        // Prepare terms for Regex: Escape chars, NO delimiters here
        $escaped_terms = array_map( function($term) {
            return preg_quote( $term, '/' );
        }, array_keys( $terms ) );
        
        // Create the term group: (TermA|TermB|TermC)
        ...
        // FIX: Removed nested delimiters. We use one outer set of delimiters /.../isu
        $pattern = '/(<a\b[^>]*>.*?<\/a>)|(<h[1-6]\b[^>]*>.*?<\/h[1-6]>)|(<script\b[^>]*>.*?<\/script>)|(<style\b[^>]*>.*?<\/style>)|(?<!\p{L})(' . $term_group . ')(?!\p{L})/isu';

        // Get current URL to prevent self-linking
        global $wp;
        $current_url = $this->get_current_url();
        return preg_replace_callback( $pattern, function( $matches ) use ( $terms, $current_url ) {
            // If groups 1-5 matched (Skip tags), return original text unchanged
            if ( ! empty($matches[1]) || ! empty($matches[2]) || ! empty($matches[3]) || ! empty($matches[4]) ) {
                return $matches[0];
            }

            // Group 5 matched! (Dictionary Word)
            $matched_word = $matches[5]; // PCRE returns the full match in [5]
            
            // Look up URL (Case-insensitive)
            foreach ( $terms as $term => $url ) {
                if ( strcasecmp( $term, $matched_word ) === 0 ) {
                    
                    // SELF-REFERENCE CHECK:
                    // If the link points to the current page, do NOT link it.
                    // (Simple check: does the permalink string contain the current slug?)
                    if ( get_permalink(get_the_ID()) === $url ) {
                        return $matched_word;
                    }
   ...
   private function get_current_url(): string {
    ...
    private function _get_post_cache( int $post_id, string $taxonomy = ''): ?string {
        if(!empty($taxonomy)){
            return get_transient( 'sparx_linked_content_' . md5($taxonomy . '_' . $post_id) );
        }
        return get_transient( 'sparx_linked_content_' . md5($post_id) );
    }
    ...
    private function get_post_cache_key(int $post_id = 0, string $taxonomy = ''): string{
        $version = defined('SPARX_3IATLAS_VERSION') ? SPARX_3IATLAS_VERSION : 'v1';
        $cache_key = md5(strval($post_id) . '_' . $version);
        if(!empty($taxonomy)){
