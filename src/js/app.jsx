...
// 3. Word Detail View (Modal/Overlay)
              ...
              <div className="space-y-4">
                {d.aiwaExampleSentences.map((ex, idx) => (
                  <div key={idx} className="pl-4 border-l-4 border-gray-200">
                    <p className="text-lg text-gray-900 mb-1">{ex.aiwaSentence}</p>
                    <p className="text-gray-500 italic">
                      {language === 'en' ? ex.aiwaSTranslationEnglish : ex.aiwaSTranslationFrench}
                    </p>
                  </div>
                ))}
              </div>
...
export default function DictionaryApp() {
  ...
  const handleScrollToLetter = (char) => {
    // Find index of first word starting with char
  ...
  if (loading) return <div className="flex h-screen items-center justify-center">Loading Dictionary...</div>;
