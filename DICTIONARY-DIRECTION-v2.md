...
**Fix:** Add `'aiwa-cpt-dictionary'` to the `register_taxonomy('starmus_tax_language')` object\_type array. This is a one-line change with a potentially large downstream effect — existing dictionary entries that have language terms already associated via the CPT admin panel may or may not have the term relationship stored correctly. After the fix, run `wp term list starmus_tax_language --orderby=count` to verify terms exist and have counts.
...
**Location:** `src/includes/Sparxstar3IAtlasPostTypes.php` registers `aiwa_sentence_ipa` (key: `field_696e6b18c17f4`) as a sub-field of the example sentences repeater with GraphQL name `sentenceIpaPronounciation`. The SCF JSON does not include this field under the repeater's sub\_fields array.
...
| `aiwa_correction_reviewed_at` | Reviewed At | date\_time\_picker | When it was reviewed |
...
- Fix `register_taxonomy('starmus_tax_language')` — add `'aiwa-cpt-dictionary'` to object\_type array  
...
| :---- | :---- |
| `AGENTS.md` | Create |
| `src/includes/Sparxstar3IAtlasPostTypes.php` | Add `aiwa-cpt-dictionary` to both taxonomy object\_type arrays (Bug 1); add `aiwa-cpt-correction` CPT \+ ACF fields |
